...
try:
    ...
    print "Failed to load the Nuke module"
...
class NukeActions(HookBaseClass):
    ...
    def _create_DO_NOT_TOUCH_NODE(self, path, sg_publish_data):


        # Creates a NoOp node
        dataCollect = nuke.nodes.NoOp(name='Published Data')

        new_knob = nuke.Text_Knob('text', 'Published File:   ')
        dataCollect.addKnob(new_knob)
        dataCollect['text'].setValue(path)



        # Collects the node's position so we can put a backdrop behind it
        x = dataCollect.xpos()
        y = dataCollect.ypos()

        # Creates a backdrop
        bd = nuke.nodes.BackdropNode(bdwidth=110, bdheight=120, name='Do Not Touch')
        bd.setXpos(x - 15)
        bd.setYpos(y - 80)
        bd['label'].setValue('DO NOT TOUCH')

        # Colors the backdrop red
        r = 1
        g = 0
        b = 0
        hexColor = int('%02x%02x%02x%02x' % (r*255, g*255, b*255, 1), 16)
        bd['tile_color'].setValue(hexColor)
